    ...
    >
      <div className="mx-auto flex w-full max-w-6xl items-center gap-4 px-4 py-3">
        <a href="#evento" className="flex items-center gap-3" onClick={() => setOpen(false)}>
          ...
          <span className="hidden text-[0.65rem] leading-tight text-white/70 sm:block">
            El Socio
            <br />
        ...
        <nav className="ml-auto hidden items-center gap-1 lg:flex">
          {NAV_LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="rounded px-2.5 py-2 text-[0.7rem] font-semibold uppercase tracking-wider text-white/75 transition hover:text-white"
            >
              {link.label}
            </a>
          ))}
      ...
      {open && (
        <nav className="border-t border-sky/15 bg-ink/95 px-4 py-3 lg:hidden">
